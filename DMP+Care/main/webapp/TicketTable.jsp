...
<head>
  ...
  <title>Ticket Table – DMP+care</title>
...
<section class="bg-gray-100 p-12">
  <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">
      <i class="fas fa-table text-red-500 mr-2"></i>Ticket Table
    </h1>
  ...
  <c:forEach var="ticket" items="${tickets}">
    				<tr class="border-b hover:bg-gray-50">
				        ...
				        <td class="px-4 py-2">${ticket.fullName}</td>
				        <td class="px-4 py-2">${ticket.email}</td>
				        <td class="px-4 py-2">${ticket.phone}</td>
				        <td class="px-4 py-2">${ticket.date}</td>
            			<td class="px-4 py-2">${ticket.time}</td>
				        <td class="px-4 py-2">${ticket.issueCategory}</td>
				        <td class="px-4 py-2">${ticket.issueDescription}</td>
					    <td class="px-4 py-2 flex gap-2">
					    <form action="editTicket.jsp " method="post" >
				            <input type="hidden" name="id" value="${ticket.ticketId}" />
				            <input type = "hidden" name="name" value="${ticket.fullName}">
				            ...
				            <input type = "hidden" name="issueCategory" value="${ticket.issueCategory}">
				            <input type = "hidden" name="issueDescription" value="${ticket.issueDescription}">
				         
				         	<input type="hidden" name="ticketId" value="${ticket.ticketId}" />
							  <button type="submit"
							     class="bg-yellow-600 text-white px-3 py-1 rounded hover:bg-yellow-500 text-xs inline-flex items-center">
						...
						<form action="DeleteTicketServlet" method="post" >
			                <input type="hidden" name="ticketId" value="${ticket.ticketId}" />
						  <button type="submit"
						      class="bg-green-600 text-white px-3 py-1 rounded hover:bg-red-600 text-xs inline-flex items-center">
			...
			<form action="TicketTable" method="post">
			<button type="submit"
			    class="bg-red-600 hover:bg-black-700 text-white text-lg px-6 py-3 rounded-lg shadow-md inline-flex items-center gap-2">
