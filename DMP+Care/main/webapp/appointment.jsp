  ...
  <div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-2xl">
    <div class="text-center mb-6">
      <h2 class="text-3xl font-bold text-gray-800">
        <i class="fas fa-calendar-check text-red-500 mr-2"></i>Book Your Appointment
      </h2>
      <p class="text-sm text-gray-500">Fill in the form to schedule a consultation</p>
    </div>

    <!-- Error placeholder (for JSP use) -->
    <% if (request.getAttribute("error") != null) { %>
      <p class="text-sm text-red-500 text-center mb-4"><%= request.getAttribute("error") %></p>
    <% } %>

    <form action="${pageContext.request.contextPath}/AddAppointment" method="POST" class="space-y-5" onsubmit= "return validateAppointmentForm()">
	...
	<!-- Health Care Provider-->
      <div>
        ...
        <input type="text" id="name" name="doctor" required
          class="w-full border border-gray-300 px-4 py-2 rounded-lg focus:ring focus:ring-blue-400" />
      </div>
